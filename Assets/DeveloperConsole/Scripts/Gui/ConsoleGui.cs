...
#pragma warning disable 0649
    ...
    using DeveloperConsole.Backend.Internal;
    using UnityEngine;
    ...
    internal class ConsoleGui
    {
        ...
        internal class Options
        {
            ...
            public bool classicRegim = false;
            public GUISkin skin;
        ...
        private bool moveCursorToEnd = false;
        ...
        internal ConsoleGui(ConsoleBackend backend, Options options)
        {
            ...
            verticalScrollBarStyle = skin.customStyles.Where(x => x.name == "verticalScroll").FirstOrDefault();
            m_backend = backend;
            m_options = options;
        ...
        {
            suggestionStyle.fontSize = fontSize + 5;
            suggestionActiveStyle.fontSize = fontSize + 5;
        ...
        private void DrawGroupCommand()
        {
            ...
            //Rect commandRect = new Rect(inputrect.x, inputrect.y, 800, 1000);
            //Rect contentRect = new Rect(inputrect.x, inputrect.y, inputrect.width, totalHeight);

            //scrollPosition = GUI.BeginScrollView(commandRect, scrollPosition, contentRect
              //  , true, true, skin.horizontalScrollbar, skin.verticalScrollbar);   



            string[] splitted = console_input.Split('.');

            var allCommands = m_backend.m_commandsTrie.GetByPrefix(console_input);

            var count = allCommands.Count();
            ...
            string[] memmory = new string[count];

            foreach (var item in allCommands)
            {
                Vector2 size = Vector2.one;
                if (item.Value.Length == console_input.Length)
                {
                    size = suggestionStyle.CalcSize(new GUIContent("ENTER"));
                    if (GUI.Button(new Rect(inputrect.x, inputrect.y + 20, size.x, size.x), "ENTER"))
                    {
                        if (!string.IsNullOrEmpty(console_input))
                        {
                            try
                ...
                string nextWord = s[splitted.Length - 1];

                if (s.Length != splitted.Length)
                    nextWord += ".";

                size = suggestionStyle.CalcSize(new GUIContent(nextWord));
                Rect pos = new Rect(inputrect.x, inputrect.y + size.y  * num * 1.3f, size.x , size.y * 1.3f);            

                if (memmory.Contains(nextWord))
                    continue;

                memmory[num] = nextWord;
        ...
        void AutoComplete2()
        {
            ...
            {
                console_input = console_input.Remove(console_input.LastIndexOf('.')).TrimEnd();
                console_input += $".{currentSuggestion}";
            }
            else
            {
